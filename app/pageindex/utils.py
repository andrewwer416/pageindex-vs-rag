...
def _scrub_thinking(content):
    """Strip <think>...</think> blocks (qwen3/r1) and surrounding whitespace."""
    if not content:
        return content
    return _THINK_RE.sub("", content).strip()


def _maybe_inject_no_think(model, messages):
    """For qwen3, append /no_think to the last user message so the model skips thinking."""

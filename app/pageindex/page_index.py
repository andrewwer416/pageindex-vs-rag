...
async def verify_toc(page_list, list_result, start_index=1, N=None, model=None):
    ...
    # Early return only if we have NO non-None physical_index at all.
    # (Upstream threshold of < len(page_list)/2 was too strict: deep documents
    # often can't be fully extrapolated from the limited toc_check_page_num
    # window, leaving back-half items with None. We still want to verify
    # whatever the model did manage to assign.)
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    ...
    if accuracy > 0.6 and len(incorrect_results) > 0:
        ...
        if mode == 'process_toc_with_page_numbers':
            ...
            # Don't recurse into process_no_toc — its prompts are even more
            # demanding (build a TOC from scratch) and small open models can't
            # reliably emit the required list-of-dicts. Return the partial
            # TOC we have; downstream filtering + summary generation handles
            # missing/imperfect physical_index gracefully.
            print(f"verify_toc accuracy={accuracy:.2f} below threshold, but returning the TOC we have rather than recursing into process_no_toc")
            return toc_with_page_number
        else:
